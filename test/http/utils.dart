...
Future<void> startServer() async {
  _server = (await HttpServer.bind('localhost', 19088))
    ..listen((request) async {
      var path = request.uri.path;
      var response = request.response;

      print("=====接收到请求 $path=== ${request.contentLength}=>");
      print("=====接收到请求 $path=== ${request.headers.contentType}=>");
      ...
      if (requestBodyBytes.isEmpty) {
        ...
        var encoding = requiredEncodingForCharset(request.headers.contentType?.charset ?? "utf-8");
        requestBody = encoding.decode(requestBodyBytes);
      } else {
        requestBody = requestBodyBytes;
      }

      var content = <String, dynamic>{'method': request.method, 'path': request.uri.path, 'headers': {}};

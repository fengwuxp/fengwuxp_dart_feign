...
class DefaultFeignClientExecutor implements FeignClientExecutor {
  ...
  DefaultFeignClientExecutor(
      this.classMirror, this.targetType, this.feignConfiguration) {
    this._initFeignClient();
  }

  @override
  Future invoke(String methodName, List<Object> positionalArguments,
    ...
    FeignRequest feignRequest = requestParamsResolver.resolve(
        positionalArguments, parameters, requestMapping.method,
        options: uiOptions);

    /// 解析url
    final requestURLResolver = feignConfiguration.requestURLResolver;
    final String requestUrl = requestURLResolver.resolve(
        this._feignClient, requestMapping, classMirror.simpleName, methodName);

    /// 解析请求头
    var requestHeaderResolver = feignConfiguration.requestHeaderResolver;
    requestHeaderResolver.resolve(
        requestMapping, feignRequest.headers, feignRequest.queryParams);
    var requestSupportRequestBody = supportRequestBody(requestMapping.method);
    if (requestSupportRequestBody == null) {
      requestHeaderResolver.resolve(
          requestMapping, feignRequest.headers, feignRequest.body);
    }

    /// TODO 解析cookie

    var responseExtractor = uiOptions.responseExtractor;
    var serializer = namedArguments[Symbol(FEIGN_SERIALIZER_PARAMETER_NAME)]
        as BuiltValueSerializable;
    if (responseExtractor == null) {
      // get response extractor
      responseExtractor = this._responseExtractor(
          requestMapping.method, serializer?.serializer,
          specifiedType: serializer?.specifiedType);
    }
    ...
    /// 执行拦截器
    feignRequest = await this
        ._preHandle(feignRequest, uiOptions, requestUrl, requestMapping);
    ...
    try {
      ...
      response = await restTemplate.execute(
          requestUrl, requestMapping.method, responseExtractor,
      ...
      // 请求失败或异常
      var result = await this._postHandleError(feignRequest, uiOptions,
          requestUrl, requestMapping, error, serializer);
      return Future.error(result);
    }

    return this._postHandle(feignRequest, uiOptions, requestUrl, requestMapping,
        response, serializer);
  }

  /// 前置拦截器
  Future _preHandle(FeignRequest request, UIOptions uiOptions, String url,
      RequestMapping requestMapping) async {
    return this._executeInterceptor<FeignBaseRequest, FeignBaseRequest>(
        request, uiOptions, url, requestMapping, request,
  ...
  Future<dynamic> _postHandle(
    ...
    return this._executeInterceptor<FeignBaseRequest, dynamic>(
        request, uiOptions, url, requestMapping, response,
        (FeignClientExecutorInterceptor<FeignBaseRequest> interceptor) {
  ...
  Future _postHandleError(
      FeignBaseRequest request,
      UIOptions uiOptions,
      String url,
      RequestMapping requestMapping,
      error,
      BuiltValueSerializable serializer) async {
    return this._executeInterceptor<FeignBaseRequest, Object>(
        request, uiOptions, url, requestMapping, error, (
            [FeignClientExecutorInterceptor<FeignBaseRequest> interceptor]) {
      if (interceptor == null) {
        return Future.error(error);
      }
      return interceptor.postError(request, uiOptions, error,
          serializer: serializer);
    }, true);
  }

  Future _executeInterceptor<T extends FeignBaseRequest, R>(
      FeignBaseRequest request,
      UIOptions uiOptions,
      String url,
      RequestMapping requestMapping,
      R defaultValue,
      ExecuteInterceptor<T> execute,
      [bool needTry = false]) async {
    final feignClientExecutorInterceptors =
        this.feignConfiguration.feignClientExecutorInterceptors;
    if (feignClientExecutorInterceptors == null) {
      return defaultValue;
    }
    var result = defaultValue,
        len = feignClientExecutorInterceptors.length,
        index = 0;
    while (index < len) {
      var feignClientExecutorInterceptor =
          feignClientExecutorInterceptors[index];
      var interceptor = this._getInterceptor(
          feignClientExecutorInterceptor, url, request.headers, requestMapping);
      if (interceptor != null) {
        if (needTry) {
          try {
  ...
  FeignClientExecutorInterceptor _getInterceptor(
      FeignClientExecutorInterceptor feignClientExecutorInterceptor,
      String url,
      Map<String, String> headers,
      RequestMapping requestMapping) {
    if (feignClientExecutorInterceptor
        is MappedFeignClientExecutorInterceptor) {
      var isMatch = feignClientExecutorInterceptor.matches(
          url: url, httpMethod: requestMapping.method, headers: headers);
      if (!isMatch) {
        return null;
      }
  ...
  ResponseExtractor _responseExtractor<T>(
      String httpMethod, Serializer<T> serializer,
...
//    }
    if (specifiedType != null && specifiedType.root == ResponseEntity) {
      return ResponseEntityResponseExtractor(
          feignConfiguration.messageConverters, serializer);
    } else {
